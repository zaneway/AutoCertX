...
GOFMT_DIRS := cmd internal/platform internal/app internal/repository internal/domain/identity internal/domain/tenancy internal/domain/job internal/domain/resource internal/domain/domains internal/domain/dnscredentials internal/domain/issuer internal/domain/certificaterequest internal/domain/certificateasset internal/domain/issueworkflow internal/domain/agentnode internal/domain/deploymenttarget internal/domain/deploymentrecord internal/domain/audit internal/domain/settings internal/application/command/auth internal/application/command/jobs internal/application/command/domains internal/application/command/caaccounts internal/application/command/certificateassets internal/application/command/nodes internal/application/command/targets internal/application/command/settings internal/application/query/authcontext internal/application/query/audit internal/application/query/dashboard internal/application/query/domains internal/application/query/jobs internal/application/query/settings internal/deployment internal/driver/acme internal/driver/agenttransport internal/driver/dns internal/agent/bootstrap internal/agent/deploy/nginx internal/agent/verify/nginx internal/workflow internal/scheduler api/openapi
...
.PHONY: prepare-go-env fmt fmt-check fmt-check-t05 lint lint-t05 test-unit test verify-ddl test-repository openapi-verify test-contracts test-auth-domain test-domain-governance test-audit-settings test-agenthub test-issuance test-nginx-deploy test-deployment test-query test-integration-auth test-integration-domain-governance test-integration-audit test-integration-query test-integration-agenthub test-integration-acme test-integration-nginx-deploy test-scheduler test-integration-scheduler run-controlplane run-agent dev-deps-up dev-deps-down dev-deps-logs web-lint web-test web-build ci-task-T00 ci-task-T01 ci-task-T02 ci-task-T03 ci-task-T04 ci-task-T05 ci-task-T06 ci-task-T07 ci-task-T08 ci-task-T09 ci-task-T10 ci-task-T13
...
test-deployment: prepare-go-env
	@GOCACHE=$(GO_BUILD_CACHE) GOTMPDIR=$(GO_TMP_DIR) $(GO) test ./internal/domain/deploymentrecord/... ./internal/deployment/...
...
test-integration-nginx-deploy: prepare-go-env
	...
	@GOCACHE=$(GO_BUILD_CACHE) GOTMPDIR=$(GO_TMP_DIR) $(GO) test ./internal/app/controlplane/http/... -run 'TestCertificateAssetDeploy'
...
ci-task-T10: fmt-check lint test-nginx-deploy test-deployment test-integration-nginx-deploy openapi-verify test-contracts
