...
- `doc/需求说明书.md` 是产品级需求来源，定义长期能力边界、非目标和演进方向。
- 本文档负责将需求收敛为一期 GA 的冻结范围、成功标准和里程碑，是一期实现的范围基线。
- `doc/一期GA详细设计.md` 以上述冻结范围为输入，继续下钻到模块边界、数据模型和接口契约。
- `doc/AI系统开发规划.md` 以本文和详细设计为输入，拆分并行研发任务、主写范围与自动化验收。
